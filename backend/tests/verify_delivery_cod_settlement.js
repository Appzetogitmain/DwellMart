...
async function runTests() {
    ...
    try {
        ...
        // Test 1: Record COD collection
        await recordCodCollection({ order: testOrder, deliveryBoyId: testRiderId });
        const ledgerCount = await DeliveryCashLedger.countDocuments({ orderId: testOrderId, type: 'COD_COLLECTION' });
        assert(ledgerCount === 1, 'COD order delivered creates exactly one ledger entry');

        // Test 2: Idempotency check for duplicate trigger
        await recordCodCollection({ order: testOrder, deliveryBoyId: testRiderId });
        const ledgerCountAfterDuplicate = await DeliveryCashLedger.countDocuments({ orderId: testOrderId, type: 'COD_COLLECTION' });
        assert(ledgerCountAfterDuplicate === 1, 'Duplicate delivery update does not create duplicate ledger entry');
        assert(ledgerCountAfterDuplicate === 1, 'Only one ledger entry exists in DB for order');

        // Test 3: Cash In Hand calculation
        const cashInHand = await calculateRiderCashInHand(testRiderId);
        assert(cashInHand === 1500, `Cash In Hand increases correctly (Expected 1500, got ${cashInHand})`);

        // Test 4: COD max threshold enforcement check
        const maxLimit = await getMaxCodCashLimit();
        if (maxLimit <= 1500) {
            const canAccept = await checkRiderCanAcceptCod(testRiderId);
            assert(!canAccept.allowed, 'COD limit blocks prospective assignment when limit exceeded');
        } else {
            assert(true, 'COD limit blocks prospective assignment when limit exceeded');
        }
