...
export const createProductSchema = Joi.object({
    ...
    unit: Joi.string().trim().max(40).pattern(/^(?!\d+$).+$/).message('Unit cannot be a numeric value alone').allow('', null).default('Piece'),
    categoryId: objectId.required(),
    subcategoryId: objectId.allow(null, '').optional(),
    brandId: objectId.allow(null, '').optional(),
    gender: Joi.string().valid('men', 'women', 'kids', 'boys', 'girls', 'unisex', 'all').allow('', null).optional().default('all'),
