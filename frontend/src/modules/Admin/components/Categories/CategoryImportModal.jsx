...
const CategoryImportModal = ({ isOpen, onClose, currentExperience = 'marketplace', onSuccess }) => {
  const [file, setFile] = useState(null);
  const [isUploading, setIsUploading] = useState(false);
  const [isDownloading, setIsDownloading] = useState(false);
  const [selectedExperience, setSelectedExperience] = useState(currentExperience);
  ...
  const handleDownloadTemplate = async () => {
    ...
    try {
      ...
      toast.success('Sample category template downloaded');
    } catch (err) {
  ...
  const handleImportSubmit = async (e) => {
    ...
    try {
      const response = await importCategoriesApi(formData);
      const data = response?.data?.data || response?.data || {};
      setImportResult(data);
      toast.success(`Categories imported: ${data.createdCount || 0} created, ${data.updatedCount || 0} updated`);
      if (onSuccess) {
        onSuccess(selectedExperience);
      }
    } catch (err) {
      const errorMessage = err?.response?.data?.message || err?.message || 'Failed to import categories';
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-900">Bulk Import Categories</h3>
              <p className="text-xs text-slate-500">Upload Excel or CSV file to create categories in bulk</p>
            </div>
          </div>
          <button
            onClick={handleClose}
        ...
        <div className="p-6 overflow-y-auto space-y-5">
          {/* Target Experience Tree */}
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-2">Target Category Tree</label>
            <div className="grid grid-cols-2 gap-3">
              {[
                { id: 'marketplace', label: '🛒 Marketplace Tree' },
                { id: 'quick_commerce', label: '⚡ Quick Commerce Tree' },
              ].map((tree) => (
                <button
                  key={tree.id}
                  type="button"
                  onClick={() => setSelectedExperience(tree.id)}
                  className={`px-4 py-2.5 rounded-xl text-sm font-semibold border transition-all text-left flex items-center justify-between ${
                    selectedExperience === tree.id
                      ? 'border-primary-500 bg-primary-50/60 text-primary-700 shadow-sm ring-2 ring-primary-500/20'
                      : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50'
                  }`}
                >
                  <span>{tree.label}</span>
                  {selectedExperience === tree.id && <FiCheckCircle className="w-4 h-4 text-primary-600" />}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                Download the pre-formatted Excel template with 3-level hierarchy examples.
              </p>
            </div>
            <button
              type="button"
